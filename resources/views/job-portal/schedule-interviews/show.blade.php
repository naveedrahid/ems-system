@extends('masterLayout.app')
@section('main')
@section('page-title')
    {{ 'Candidate' }}
@endsection
@section('page-content')
    <div class="card-body profile-box">
        <div class="row">
            <div class="col-md-6">
                <div class="card small-box profile-info">
                    <div class="card-header">
                        <h4 class="text-bold">Candidate Personal Info</h4>
                    </div>
                    <div class="card-body p-0 mb-3">
                        <div class="table-responsive">
                            <table class="table m-0">
                                <tbody class="menual-update-1">
                                    <tr>
                                        <td class="text-bold">First Name</td>
                                        <td>{{ $candidate->first_name ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Last Name</td>
                                        <td>{{ $candidate->last_name ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Age</td>
                                        <td>{{ $candidate->age ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">City</td>
                                        <td>{{ $candidate->city ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Gender</td>
                                        <td>{{ $candidate->gender ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Address</td>
                                        <td>=</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Country</td>
                                        <td>=</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Marital Status</td>
                                        <td>{{ $candidate->marital_status ?? '' }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>

                <div class="card small-box profile-info">
                    <div class="card-header">
                        <h4 class="text-bold">Contact Info</h4>
                    </div>
                    <div class="card-body p-0 mb-3">
                        <div class="table-responsive">
                            <table class="table m-0">
                                <tbody class="menual-update-1">
                                    <tr>
                                        <td class="text-bold">Email</td>
                                        <td>{{ $candidate->email ?? ''}}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Phone</td>
                                        <td>{{ $candidate->phone ?? ''}}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                <div class="card small-box profile-info">
                    <div class="card-header">
                        <h4 class="text-bold">Attachments</h4>
                    </div>
                    <div class="card-body p-0 mb-3">
                        <div class="table-responsive">
                            <table class="table m-0">
                                <tbody class="menual-update-1">
                                    <tr>
                                        <td class="text-bold">Resume</td>
                                        <td><a href="{{ asset($candidate->resume) }}" target="_blank">View
                                                Resume</a></td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Cover Letter</td>
                                        <td>
                                            <a href="{{ asset($candidate->cover_letter) }}" target="_blank">Download
                                                Cover Letter</a>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                <a href="{{route('schedule-interviews.index')}}" class="btn btn-danger">Back</a>
            </div>
            <div class="col-md-6">
                <div class="card small-box profile-info">
                    <div class="card-header">
                        <h4 class="text-bold">Job Description</h4>
                    </div>
                    <div class="card-body p-0 mb-3">
                        <div class="table-responsive">
                            <table class="table m-0">
                                <tbody class="menual-update-1">
                                    <tr>
                                        <td class="text-bold">Total Experience</td>
                                        <td>{{ $candidate->total_experience ?? '' }}year</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Current Salary</td>
                                        <td>${{ number_format($candidate->current_salary, 0, '.', ',') ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Expected Salary</td>
                                        <td>${{ number_format($candidate->expected_salary, 0, '.', ',') ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Notice Period</td>
                                        <td>{{ $candidate->notice_period ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-bold">Switching Reason</td>
                                        <td>{{ $candidate->switching_reason ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        @php
                                            $date = new DateTime($candidate->datetime);
                                            $formattedDate = $date->format('l d M g:iA');
                                        @endphp
                                        <td class="text-bold">Your Available Date and Time</td>
                                        <td>{{ $formattedDate ?? ''}}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                <div class="card small-box profile-info">
                    <div class="card-header">
                        <h4 class="text-bold">Social Info</h4>
                    </div>
                    <div class="card-body p-0 mb-3">
                        <div class="table-responsive">
                            <table class="table m-0">
                                <tbody class="menual-update-1">
                                    @if ($candidate->linkdin)
                                        <tr>
                                            <td class="text-bold">LinkedIn</td>
                                            <td><a href="{{ $candidate->linkdin ?? '' }}"
                                                    target="_blank">{{ $candidate->linkdin ?? '' }}</a></td>
                                        </tr>
                                    @endif
                                    @if ($candidate->github)
                                        <tr>
                                            <td class="text-bold">GitHub</td>
                                            <td><a href="{{ $candidate->github ?? '' }}"
                                                    target="_blank">{{ $candidate->github ?? '' }}</a></td>
                                        </tr>
                                    @endif
                                    @if ($candidate->behance)
                                        <tr>
                                            <td class="text-bold">Behance</td>
                                            <td><a href="{{ $candidate->behance ?? '' }}"
                                                    target="_blank">{{ $candidate->behance ?? '' }}</a></td>
                                        </tr>
                                    @endif
                                    @if ($candidate->website)
                                        <tr>
                                            <td class="text-bold">Website</td>
                                            <td><a href="{{ $candidate->website ?? '' }}"
                                                    target="_blank">{{ $candidate->website ?? '' }}</a>
                                            </td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
@endsection

@push('css')
<style>
    .profile-box table tr {
        font-size: 16px !important;
    }
</style>
@endpush
